...
def build(root: Path = ROOT, local: bool = False, user_skills: Path = USER_SKILLS, plugins: Path = PLUGINS,
    ...
    L = ["---", "name: capabilities",
         "description: Master index of EVERYTHING the agent can do here - harnessd skills, your repo skills, the full ECC "
         ...
         f"`{HARNESS_TOKEN}` = the harnessd engine directory (`harnessd doctor` prints it)." if not local else
    ...
    L += ["## Loaded: harnessd core", "", *table(core), ""]
